...
class trackingPartials:
    ...
    def __init__(self, y , fs , N = 2^11-1 , hopFactor=4 , overSample=2 , G_g=-40 , Q=2 , delta=0.2 , zetaF = 50 , zetaA = 15):
        ...
        self.zetaA = max(1e-1, zetaA)/20 * np.log(10) #ln(Za_amplitude)
        self.delta = delta 
    ...
    def setup(self) : 
        ...
        # self.win = functions.hann_window(self.N)
        # self.winD = functions.hann_window(self.N,derivative=2)
        # self.hopFactor = max(4,self.hopFactor)


        # self.win = hann(self.N,False)
        ...
        #self.ypad = np.concatenate((self.y, np.zeros(self.padding[1])))
        #self.ypad = self.y
        ...
        self.omega = 2 * np.pi * ndft/Ndft
        #t = np.arange(self.N) / self.fs

        n_center = int((self.N-self.N%2)/2 + 1) 
        self.centering = np.exp(1j * self.omega * n_center)

        n = np.expand_dims(np.arange(self.N),axis=1) - n_center
        self.ft_mat = np.exp(1j*np.outer(n , self.omega[:Ndft//2].T.conj()))
        ...
        self.p = functions.time_vector_computation(n,self.Q)
        self.pD = functions.derivative_time_vector_computation(n,self.Q)
    ...
    def _costMatrixComputation(self,Alpha_last,Alpha):
        ...
        parameter_test = functions.FreqAmpPhadAmp(self.fs,Alpha_last, time2, dTime2)
        parameter_test2 = functions.FreqAmpPhadAmp(self.fs,Alpha, time1, dTime1)
    ...
    def _parameterEstimation(self,signal):
        ...
        Alpha, num_peaks, S = ddm.ddm(signal, self.Q, Rddm, self.G_g, self.win, self.winD, self.p, self.pD, self.centering, self.Ndft, self.ft_mat, self.omega)
            ...
            if ( m > 0 ) : # we cannot track the first frame
                ...
                if num_peaks > 0 and num_peaks_last > 0 :
                    ...
                    if num_assignement > 0 : 
                        ...
                        Partials[m-1][ii_active_last, 0:4] = functions.FreqAmpPhadAmp(self.fs,Alpha_last[:, Assignments[toBirth, 0]], p, pD)
                        Partials[m-1][ii_active_last, 4] = tracks_[num_toContinue:num_active]

                        Partials[m][0:num_active, 0:4] = functions.FreqAmpPhadAmp(self.fs, Alpha[:, pairs_], p, pD)
